...
public partial class SettingsService(ApolloDbContext context, ILogger<SettingsService> logger) : ISettingsService
{
  private readonly ApolloDbContext _context = context;
  ...
  public async Task<bool> GetBooleanSettingAsync(string key, bool defaultValue = false)
  {
    ...
    return lowerValue is "1" or "yes" or "on" or "enabled" || defaultValue;
  }

  public async Task<int> GetIntegerSettingAsync(string key, int defaultValue = 0)
  {
    var value = await GetSettingAsync(key);
    return !string.IsNullOrWhiteSpace(value) && int.TryParse(value, out int result) ? result : defaultValue;
  }
  ...
  {
    return await SetSettingAsync(key, value.ToString(System.Globalization.CultureInfo.InvariantCulture));
  }
  ...
  public async Task<bool> SetSettingAsync(string key, string value)
  {
    ...
    try
    {
      Setting? existingSetting = await _context.Settings
          .FirstOrDefaultAsync(s => s.Key == key);
      ...
      else
      {
        ...
        _ = _context.Settings.Add(newSetting);
  ...
  public async Task<bool> DeleteSettingAsync(string key)
  {
    ...
    try
    {
      ...
      _ = _context.Settings.Remove(setting);
      _ = await _context.SaveChangesAsync();
