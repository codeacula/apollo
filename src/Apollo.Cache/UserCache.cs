...
public sealed class UserCache(IConnectionMultiplexer redis, ILogger<UserCache> logger) : IUserCache
{
  private readonly IDatabase _db = redis.GetDatabase();
  private readonly ILogger<UserCache> _logger = logger;
  private const string KeyPrefix = "user:access:";
  ...
  public async Task<Result> SetUserAccessAsync(Username username, bool hasAccess, CancellationToken cancellationToken = default)
  {
    try
    {
      var key = GetCacheKey(username);
  ...
  public async Task<Result> InvalidateUserAccessAsync(Username username, CancellationToken cancellationToken = default)
  {
    try
    {
      var key = GetCacheKey(username);
