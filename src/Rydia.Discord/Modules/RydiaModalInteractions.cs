...
public partial class RydiaModalInteractions(
    ILogger<RydiaModalInteractions> logger,
    ISettingsService settingsService,
    ISettingsProvider settingsProvider) : ComponentInteractionModule<ModalInteractionContext>
{
    private readonly ILogger<RydiaModalInteractions> _logger = logger;
    ...
    public async Task ConfigureDailyAlertTimeAsync()
    {
        ...
        {
            // Save time
            var timePersisted = await _settingsService.SetSettingAsync(RydiaSettings.Keys.DailyAlertTime, timeInput);
            
            // Save message
            var messagePersisted = await _settingsService.SetSettingAsync(RydiaSettings.Keys.DailyAlertInitialMessage, messageInput);
