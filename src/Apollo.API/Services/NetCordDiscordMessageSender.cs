...
/// <summary>
/// Sends Discord messages to the daily alert channel using NetCord REST client.
/// </summary>
public partial class NetCordDiscordMessageSender(ISettingsProvider settingsProvider, RestClient restClient, ILogger<NetCordDiscordMessageSender> logger) : IDiscordMessageSender
{
    ...
    public async Task<(bool Success, string? Error, ulong? MessageId)> SendToDailyAlertAsync(string content, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var props = new MessageProperties() { Content = content };
            var message = await _restClient.SendMessageAsync(channelId.Value, props, null, ct);
    ...
    public async Task<(bool Success, string? Error, ulong? ThreadId, ulong? MessageId)> CreateForumPostAsync(ulong forumChannelId, string title, string content, IEnumerable<ulong>? appliedTagIds, CancellationToken ct)
    {
        try
        {
            ...
            var message = new ForumGuildThreadMessageProperties { Content = content };
            var threadProps = new ForumGuildThreadProperties(title, message)
                .WithAppliedTags(appliedTagIds);

            var thread = await _restClient.CreateForumGuildThreadAsync(forumChannelId, threadProps, null, ct);
